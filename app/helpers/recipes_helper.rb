  ...
  def recipe_card_image(user, recipe)
    if recipe.image.attached?
      ...
      "<div class='outer-container'>
        <div class='image-container'>
          #{recipe_liked_icon(user, recipe)}
          #{image_tag url_for('no-photo-icon-28.jpg'), class: 'card-img-top image'}
        </div>
      </div>"
      .html_safe           
    end
  ...
  def recipe_tile_image(user, recipe)
    if recipe.image.attached?
      ...
      "<div class='outer-container'>
        <div class='image-container'>
          #{recipe_liked_icon(user, recipe)}
          #{image_tag url_for('no-photo-icon-28.jpg'), class: 'image', style: 'height: 200px'}
        </div>
      </div>"
      .html_safe      
    end
  ...
  def recipe_info_image(user, recipe)
    if recipe.image.attached?
      ...
      "<div class='outer-container'>
        <div class='image-container'>
          #{recipe_liked_icon(user, recipe)}
          #{image_tag url_for('no-photo-icon-28.jpg'), class: 'image', style: 'width: 100%'}
        </div>
      </div>"
      .html_safe           
    end
